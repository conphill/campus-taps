module Webadmin::PostsHelper
  
  def add_photo(form_builder)
    link_to_function "add", :id  => "add_photo" do |page|
      form_builder.fields_for :blog_photos, BlogPhoto.new, :child_index => 'NEW_RECORD' do |photo_form|
        html = render(:partial => 'webadmin/posts/photo', :locals => { :f => photo_form })
